...
class FeaturePoints(Node):
    ...
    def build_J_cost(Top , p , y, epsilon):
        from pyecca.lie import se3
        ...
        epsilon: symbolic pertubation 
        """
        
        # Form symbolic transformation matrix using pyecca
        SE3 = se3._SE3()
        T_sym = SE3.exp(SE3.wedge(epsilon)) #symbolic
                        ...
                        if self.pc_prev is not None:
                            ...
                            if len(xyz_points) > 2:
                                if self.img_prev_ is not None:
                                    ...
                                    while algopt is None or ca.norm_2(algopt)>1e-4:  

                                        #cost function (Barfoot 8.95)
                                        J = BF_PCA.build_J_cost(   Top=self.Top_, 
                                                                    p = xyz_points_prev, 
                                                                    y = xyz_points, 
                                                                    epsilon = epsilon)
                                        ...
                                        # x_input = np.zeros(6)
                                        optim = F(x0=x_input)
                                        algopt = np.array(optim['x'])
                                        #Top_ should be a numpy type
