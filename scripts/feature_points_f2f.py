...
class FeaturePoints(Node):
    ...
    def barfoot_solve(self, Top, p, y):
        ...
        M1 = np.vstack((np.hstack((np.eye(3), np.zeros([3,3]))),
                         np.hstack((self.SO3.wedge(P[0]),np.eye(3)))))
        M2 = np.vstack((np.hstack((np.eye(3), np.zeros([3,3]))), 
                        np.hstack((np.zeros([3,3]),I))))
                    ...
                    # if True:
                        ...
                        if self.pc_prev is not None:
                            ...
                            while ransacking ==True:
                                ...
                                counter = 0
                                algopt=None
                                while (algopt is None or np.linalg.norm(algopt)>1e-8) and counter<900:    
                                ...
                                if counter2 >10:
                                    ...
                                    return
                            #print('ransac_Top',self.ransac_T)
                            ...
                            if len(xyz_points) > 2:
                                ...
                                if self.img_prev_ is not None:
                                    ...
                                    #  and counter<200
                                    while (algopt is None or np.linalg.norm(algopt)>1e-8):
                                    ...
                                    self.Top_cum_ = self.Top_@self.Top_cum_
                                    print("self.Top_cum: ", self.Top_cum_)                                   
                                    q = tf_transformations.quaternion_from_matrix(self.Top_cum_)

                                    t_cum = self.Top_cum_[0:3,3]
                                    print(self.Top_cum_[:3,:3])
                                    print(q)
                                    t_act = -self.Top_cum_[:3,:3].T@t_cum
                                    #print('t_act :', t_act)
                                    msg = PoseStamped()
