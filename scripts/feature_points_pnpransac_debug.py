...
class FeaturePoints(Node):
    ...
    def __init__(self):
        ...
        self.k_=  np.array([[611.2769775390625,0.0,434.48028564453125],
                            [0.0,609.7720336914062,237.57313537597656],
                            [0.0, 0.0, 1.0]])
        
        # from /camera/color/camera_info
        # k:
        # - 611.2769775390625
        # - 0.0
        # - 434.48028564453125
    ...
    def listener_callback(self, msg):
        ...
        if method == 'surf':
            ...
            # Catches if zero features are calculated.
                ...
                #print('number of mathces',len(matches))
                ...
                if len(matches) > 1:
                    ...
                    if True:#(len(kp) < self.nfeatures) and not (len(self.kp_prev_) < self.nfeatures):
                    # if True:
                        ...
                        if self.pc_prev is not None:
                            ...
                            #Revert points in 'previous' frame to inertial frame
                            ...
                            for lcv in range(len(xyz_points)):
                                ...
                                # Upper tolerance
                                    ...
                                    valid_pxls.append(list_pxl[lcv])
                            ...
                            xyz_prev_hold = xyz_points_prev

                            # xyz_points_prev = camera_link_R@xyz_points_prev.T
                            # xyz_points_prev=xyz_points_prev
                            # xyz_points = camera_link_R@xyz_points.T
                            ...
                            #print("num pixels: ", len(valid_pxls))
                            _, rvec, tvec, _ = cv2.solvePnPRansac(np.vstack(xyz_prev_hold), np.array(valid_pxls).astype(np.float32), self.k_.astype(np.float32), None)
                            rmat = np.zeros(shape=(3,3))
                            cv2.Rodrigues(rvec, rmat)
                            # print("rmat", rmat)
                            rmat = rmat
                            tvec= -rmat@tvec
                            self.ransac_T = np.vstack((rmat,[0,0,0]))
                            self.ransac_T = np.hstack((self.ransac_T,np.vstack((tvec,1))))
                            self.ransac_T=self.ransac_T@self.Top_cum_
                            ...
                            if len(xyz_clean) > 2:
                                ...
                                if self.img_prev_ is not None:
                                    ...
                                    #Barfoot 'M' matrix based on the previous points
                                    P = np.expand_dims(np.average(xyz_points_prev,axis=0), axis=0)
                                    I = 0
                                    for j in range(len(xyz_points_prev)):
                                        pint0 = xyz_points_prev[[j],:] - P
                                        I += self.SO3.wedge(pint0[0])@self.SO3.wedge(pint0[0])
                                    I=-I/len(xyz_points_prev)
                                    ...
                                    while (algopt is None or np.linalg.norm(algopt)>1e-10):    
                                        algopt = self.barfoot_solve(self.Top_, xyz_points_prev, xyz_points, M)
                                    ...
                                    # print(t_rot.shape)
                                    t_act = -self.Top_cum_[:3,:3].T@t_rot
